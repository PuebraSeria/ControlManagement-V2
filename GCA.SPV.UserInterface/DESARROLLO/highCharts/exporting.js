...
(
    function (f) {
        var A = f.Chart, t = f.addEvent, B = f.removeEvent, l = f.createElement, o = f.discardElement, v = f.css, k = f.merge, r = f.each, p = f.extend, D = Math.max, j = document, C = window, E = f.isTouchDevice, F = f.Renderer.prototype.symbols, s = f.getOptions(), y; p(s.lang, { printChart: "Imprimir gráfico", downloadPNG: "Descargar como PNG", downloadJPEG: "Descargar como JPEG", downloadPDF: "Descargar como PDF", contextButtonTitle: "Chart context menu" }); s.navigation = {
            menuStyle: {
